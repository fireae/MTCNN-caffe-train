...
void MTCNNEuclideanLossLayer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void MTCNNEuclideanLossLayer<Dtype>::Forward_gpu(
    const vector<Blob<Dtype>*>& bottom, const vector<Blob<Dtype>*>& top) {
  int count = bottom[0]->count();
  int has_ignore_label = this->layer_param().loss_param().has_ignore_label();
  int ignore_label =
      has_ignore_label ? this->layer_param().loss_param().ignore_label() : -1;
  ...
  if (has_ignore_label) {
    ...
    for (int i = 0; i < countLabel; ++i) {
      if (label[i] != ignore_label) {
        caffe_gpu_sub(channel, b0 + i * channel, b1 + i * channel,
                      diff + i * channel);
        Dtype dot;
...
template <typename Dtype>
void MTCNNEuclideanLossLayer<Dtype>::Backward_gpu(
  ...
  if (has_ignore_label) {
    ...
    for (int i = 0; i < 2; ++i) {
      if (propagate_down[i]) {
        caffe_gpu_memset(sizeof(Dtype) * bottom[i]->count(), 0,
        ...
        for (int j = 0; j < countLabel; ++j) {
          ...
          if (label[j] != ignore_label) {
            caffe_gpu_axpby(channels,                         // count
                            alpha,                            // alpha
